...
router.post(
  ...
  (req, res, next) => {
    ...
    papers.filename = res.req.file.filename;

    papers.save().then(newPaper => {
      console.log(newPaper);
      res.status(201).json({ ...newPaper.toObject(), id: newPaper._id });
    });
  }
);
...
router.get("", (req, res, next) => {
  var docs = [];
  let docslength;
  Papers.find().then(documents => {
    docslength = documents.length;
    documents.map(papers => {
      User.findById({ _id: papers.creator }).then(user => {
        if (user) {
          user1 = {
            username: user.name,
            designation: "Student",
          ...
          user1 = Object.assign(user1, papers.toObject());
          docs.push(user1);
          ...
          Faculty.findById({ _id: papers.creator }).then(faculty => {
            faculty = {
              username: faculty.name,
              designation: "Faculty"
            };
            faculty = Object.assign(faculty, papers.toObject());
            docs.push(faculty);
            console.log(docs.length);
            if (docs.length === docslength) {
              // console.log(docs);
...
router.post("/search", (req, res, next) => {
  ...
  Papers.search(req.body.search, (findErr, findRes) => {
    if (findErr) {
      //log error here
      res.status(500).send({
