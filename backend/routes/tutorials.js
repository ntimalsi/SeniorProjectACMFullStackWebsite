...
router.post(
  ...
  (req, res, next) => {
    ...
    tutorial.save().then(newTutorial => {
      ...
      res.status(201).json({ ...newTutorial.toObject(), id: newTutorial._id });
    });
  }
);
...
router.get("", (req, res, next) => {
  ...
  Tutorial.find().then(documents => {
    ...
    documents.map(tutorial => {
      User.findById({ _id: tutorial.creator }).then(user => {
        if (user) {
          user1 = {
            username: user.name,
            designation: "Student",
          ...
          user1 = Object.assign(user1, tutorial.toObject());
          docs.push(user1);
          ...
          Faculty.findById({ _id: tutorial.creator }).then(faculty => {
            faculty = {
              username: faculty.name,
              designation: "Faculty"
            };
            faculty = Object.assign(faculty, tutorial.toObject());
            docs.push(faculty);
            console.log(docs.length);
            if (docs.length === docslength) {
              // console.log(docs);
...
router.post("/search", (req, res, next) => {
  ...
  Tutorial.search(req.body.search, (findErr, findRes) => {
    if (findErr) {
      //log error here
      res.status(500).send({
