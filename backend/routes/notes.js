...
router.post(
  ...
  (req, res, next) => {
    ...
    notes.filename = res.req.file.filename;

    notes.save().then(newNotes => {
      console.log(newNotes);
      res.status(201).json({ ...newNotes.toObject(), id: newNotes._id });
    });
  }
);
...
router.get("", (req, res, next) => {
  var docs = [];
  let docslength;
  Notes.find().then(documents => {
    docslength = documents.length;
    documents.map(notes => {
      User.findById({ _id: notes.creator }).then(user => {
        if (user) {
          user1 = {
            username: user.name,
            designation: "Student",
          ...
          user1 = Object.assign(user1, notes.toObject());
          docs.push(user1);
          ...
          Faculty.findById({ _id: notes.creator }).then(faculty => {
            faculty = {
              username: faculty.name,
              designation: "Faculty"
            };
            faculty = Object.assign(faculty, notes.toObject());
            docs.push(faculty);
            console.log(docs.length);
            if (docs.length === docslength) {
              // console.log(docs);
...
router.post("/search", (req, res, next) => {
  ...
  Notes.search(req.body.search, (findErr, findRes) => {
    if (findErr) {
      //log error here
      res.status(500).send({
