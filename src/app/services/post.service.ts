...
export class PostService {
  ...
  addPost(
    ...
    this.http
      .post<{ post; postid: string }>(
        'http://turing.cs.olemiss.edu:5001/api/posts',
        newpost
      )
      .subscribe(resdata => {
  ...
  getPosts(): Observable<any> {
    return this.http.get<[]>('http://turing.cs.olemiss.edu:5001/api/posts');
    // .pipe
    // map(postData => {
  ...
  updatePost(
    ...
    console.log('at 3');
    this.http
      .put('http://turing.cs.olemiss.edu:5001/api/posts/' + id, newpost)
      .subscribe(() => {
        this.reloadComponent();
      });
  }

  public postUser = new Subject<any>();
  private pUser;

  postUserSub(userid: string) {
    this.getPostUser(userid);
    return this.postUser.asObservable();
  }

  postUserNoSub(userid: string) {
    this.getPostUser(userid);
    return this.pUser;
  }

  getPostUser(userid: string): Observable<any> {
    return this.http.get<{
      name: string;
      designation: string;
      profileimg: string;
    }>('http://turing.cs.olemiss.edu:5001/api/posts/postUserInfo/' + userid);
    // .subscribe(result => {
    //   const puser = {
    ...
    //   };
    //   this.pUser = puser;
    //   this.postUser.next(puser);
    // });
  }

  deletePost(postId: string) {
    this.http
      .delete<{ message: string }>('http://turing.cs.olemiss.edu:5001/api/posts/' + postId)
      .subscribe(message => {
        this.reloadComponent();
      });
  }
